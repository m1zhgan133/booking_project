...
def create_app(test_config=None):
    ...
    def update_booking_():
        ...
        if (user and user['name'] == username and user['password'] == password) or is_admin(username, password):
            ...
            if is_updated:
                ...
                return error_response('Проблемы с редактированием брони', 400)
        else:
    ...
    def delete_booking():
        ...
        if (user and user['name'] == username and user['password'] == password) or is_admin(username, password):
            if delete_booking_by_id(booking_id):
                return jsonify({"message": 'Бронь успешно удалена'}), 204
            else:
    ...
    def update_user_():
        ...
        if username and password and is_admin(username, password):
            ...
            return error_response('У вас нет прав администратора, для совершения этого действия', 403)
    ...
    def delete_user_():
        ...
        if username and password and is_admin(username, password):
            ...
            if delete_user(user_id):
                ...
                return error_response('Проблемы с удалением пользователя',400)
        else:
            return error_response('У вас нет прав администратора, для совершения этого действия', 403)
    # ---------------------------- admin check ----------------------------
